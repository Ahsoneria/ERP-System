...
class Window(object):
    def __init__(self,window):
        self.window = window
        self.window.wm_title("Location Screen")
        ...
        l10 = Label(window, text="Location Screen", font=myFont_3)
        ...
        l0 = Label(window, text="Location ID", font=myFont)
        l0.grid(row=0, column=0, pady=20)
        l0.config(bg="burlywood4", fg="black")

        l1 = Label(window, text="Location Name", font=myFont)
        l1.grid(row=1, column=0, pady=20)
        l1.config(bg="burlywood4", fg="black")

        l2 = Label(window, text="Product Category ID", font=myFont)
        ...
        self.procat_id_text = StringVar()
        self.e2 = Entry(window, textvariable=self.procat_id_text, font=myFont)
        ...
        b7.grid(row=1, column=3,padx=20, pady=20)
    ...
    def search_command(self):
        self.list1.delete(0, END)
        for row in database.search(self.id_text.get(), self.name_text.get(), self.procat_id_text.get()):
            self.list1.insert(END, row)

    def add_command(self):
        database.insert(self.id_text.get(), self.name_text.get(), self.procat_id_text.get())
        self.list1.delete(0, END)
        self.list1.insert(END, (self.id_text.get(), self.name_text.get(), self.procat_id_text.get()))

    def delete_command(self):
        database.delete(self.selected_tuple[0],self.selected_tuple[2])
        self.view_command()

    def update_command(self):
        #be careful for the next line ---> we are updating using the texts in the entries, not the selected tuple
        database.update(self.selected_tuple[0], self.name_text.get(), self.procat_id_text.get(),self.selected_tuple[2])
        self.view_command()
